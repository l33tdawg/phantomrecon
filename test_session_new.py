    ...
    try:
        async for event in runner.run_async(
            ...
            if not event.partial:
                # Try different attributes to find the right one
                agent_identifier = getattr(event, "name", None)
                if agent_identifier is None:
                    agent_identifier = getattr(event, "agent", None)
                if agent_identifier is None:
                    agent_identifier = "Unknown agent"
                
                logger.info(f"Event from {agent_identifier}: {event.text}")
    except Exception as e:
        logger.error(f"Error during agent execution: {e}")
        # Continue with the test to see if state was still maintained
