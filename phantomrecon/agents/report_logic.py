...
def _build_markdown_report(recon_data: Dict, attack_plan: Dict, exploit_results: List[Dict], summary_text: str) -> str:
    ...
    # Include DNS/WHOIS info if available (with fallbacks for older/newer schemas)
    dns_info = recon_data.get('dns_recon', {})
    if dns_info and not dns_info.get("error"):
         report.append("### DNS/WHOIS Info:")
         # Prefer nested dns.dig if present, else fall back to flat dns_records
         dig_results = dns_info.get('dns', {}).get('dig', {})
         if not dig_results and isinstance(dns_info.get('dns_records'), dict):
              dig_results = dns_info.get('dns_records')
         # Records
         ...
         if isinstance(whois_data, str):
              if whois_data.lower().startswith("failed") or whois_data.lower().startswith("skipped"):
                   whois_status = whois_data
              elif len(whois_data) > 10:
                   whois_status = "Available (details omitted)"
         elif isinstance(whois_data, dict) and whois_data:
              # If structured whois exists, consider it available
              whois_status = "Available (details omitted)"
         report.append(f"- WHOIS Status: {whois_status}")
         # AXFR summary if present
         axfr_attempt = dns_info.get('dns', {}).get('axfr_attempt') or dns_info.get('axfr_attempt')
         if isinstance(axfr_attempt, dict) and 'status' not in axfr_attempt:
              successful_axfr = [ns for ns, res in axfr_attempt.items() if isinstance(res, dict) and res.get('status') == 'success']
              if successful_axfr:
         ...
         # DNS errors if any
         if isinstance(dns_info.get("errors"), list) and dns_info["errors"]:
    ...
    if isinstance(attack_plan, dict) and not attack_plan.get("error"):
        ...
        for target_key, info in attack_plan.items():
            if target_key in ['error', 'details']:
                continue
            # Special-case subdomains list structure
            if target_key == 'subdomains' and isinstance(info, dict) and 'targets' in info:
                plan_items_count += 1
                subs = info.get('targets') or []
                report.append(f"- **Target Key:** `subdomains` ({len(subs)} discovered)")
                names = []
                for s in subs:
                    if isinstance(s, dict) and s.get('name'):
                        names.append(s.get('name'))
                if names:
                    report.append(f"  - Names: {', '.join(names[:10])}{'...' if len(names)>10 else ''}")
            ...
            if isinstance(info, dict):
                host = info.get('target_host') or target
                port = info.get('port', 'unknown')
                host_port = f"{host}:{port}"
                # Build a meaningful service string from available fields
                if info.get('service_name') or info.get('product'):
                    service = f"{info.get('service_name', 'unknown')} ({info.get('product', 'unknown')} {info.get('version', 'unknown')})"
                elif info.get('version'):
                    service = str(info.get('version'))
                elif info.get('notes'):
                    service = str(info.get('notes'))
                else:
                    service = "Unknown"

                report.append(f"- **Target Key:** `{target_key}`")
                report.append(f"  - Host: {host_port}")
                ...
                for sub_info in info:
                    if isinstance(sub_info, dict):
                        host = sub_info.get('target_host') or target
                        port = sub_info.get('port', 'unknown')
                        host_port = f"{host}:{port}"
                        if sub_info.get('service_name') or sub_info.get('product'):
                            service = f"{sub_info.get('service_name', 'unknown')} ({sub_info.get('product', 'unknown')} {sub_info.get('version', 'unknown')})"
                        else:
                            service = str(sub_info.get('version') or sub_info.get('notes') or 'Unknown')
                        report.append(f"  - Host: {host_port}")
                        report.append(f"    - Service: {service}")
                        tests = sub_info.get('tests') or []
                        if tests:
                            report.append("    - Planned Tests:")
                            for test in tests:
                                report.append(f"      - `{test}`")
